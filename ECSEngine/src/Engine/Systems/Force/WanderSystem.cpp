...
void WanderSystem::Update(Coordinator& coordinator, float dt, double simulationTime)
{
    ...
    ParallelFor(totalEntities, [this, &coordinator, simulationTime, &entities, &wanderArray, &forceArray](int startIdx, int endIdx) {
        for (int i = startIdx; i < endIdx; ++i) {
            Entity entity = entities[i];
            ProcessEntity(entity, coordinator, simulationTime, wanderArray.GetData(entity), forceArray.GetData(entity));
        }
    });
}
