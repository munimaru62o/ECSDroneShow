...
void LiftSystem::Update(Coordinator& coordinator, float dt, double simulationTime)
{
    ...
    ParallelFor(totalEntities, [this, &entities, &transformArray, &liftArray, &forceArray](int startIdx, int endIdx) {
        for (int i = startIdx; i < endIdx; ++i) {
            Entity entity = entities[i];
            ProcessEntity(entity, transformArray.GetData(entity), liftArray.GetData(entity), forceArray.GetData(entity));
        }
    });
}
