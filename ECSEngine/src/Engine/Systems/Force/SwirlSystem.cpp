...
void SwirlSystem::Update(Coordinator& coordinator, float dt, double simulationTime)
{
    ...
    ParallelFor(totalEntities, [this, &entities, &transformArray, &swirlArray, &forceArray](int startIdx, int endIdx) {
        for (int i = startIdx; i < endIdx; ++i) {
            Entity entity = entities[i];
            ProcessEntity(entity, transformArray.GetData(entity), swirlArray.GetData(entity), forceArray.GetData(entity));
        }
    });
}
...
void SwirlSystem::ProcessEntity(Entity entity, const TransformComponent& transform, const SwirlComponent& swirl, ForceComponent& force) const
{
    ...
    if (distSq > MathConstants::ZERO_TOLERANCE) {
        ...
        DEBUG_DRAW_FORCE(entity, transform.position, forceDir, Debug::DrawColor::Force::Swirl);
    }
}
