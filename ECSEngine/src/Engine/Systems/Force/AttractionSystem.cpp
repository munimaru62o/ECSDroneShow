...
void AttractionSystem::Update(Coordinator& coordinator, float dt, double simulationTime)
{
    ...
    ParallelFor(totalEntities, [this, &entities, &transformArray, &attractionArray, &forceArray](int startIdx, int endIdx) {
        for (int i = startIdx; i < endIdx; ++i) {
            Entity entity = entities[i];
            ProcessEntity(entity, transformArray.GetData(entity), attractionArray.GetData(entity), forceArray.GetData(entity));
        }
    });
}
