...
void OrbitSystem::Update(Coordinator& coordinator, float dt, double simulationTime)
{
    ...
    ParallelFor(totalEntities, [this, &coordinator, simulationTime, &entities, &targetArray, &orbitArray](int startIdx, int endIdx) {
        for (int i = startIdx; i < endIdx; ++i) {
            Entity entity = entities[i];
            ProcessEntity(entity, coordinator, simulationTime, targetArray.GetData(entity), orbitArray.GetData(entity));
        }
    });
}
