...
public:
    ...
    void RegisterSingleton(RegistrationPolicy policy = RegistrationPolicy::Forbid)
    {
        static_assert(!std::is_reference_v<Interface> && !std::is_const_v<Interface> && !std::is_volatile_v<Interface>, "Interface must be an unqualified non-reference type.");
        static_assert(!std::is_reference_v<Implementation> && !std::is_const_v<Implementation> && !std::is_volatile_v<Implementation>, "Implementation must be an unqualified non-reference type.");
    ...
    {
        static_assert(!std::is_reference_v<T> && !std::is_const_v<T> && !std::is_volatile_v<T>, "RegisterInstance requires an unqualified non-reference service type (Cannot register 'const' instances).");
