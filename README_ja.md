...
本プロジェクトはビルドシステムに CMake を採用しています。Visual Studio のネイティブ CMake サポートを利用することで、簡単に環境を構築できます。

1. **リポジトリの取得**
   - 本リポジトリを任意のディレクトリにクローン（またはダウンロードして解凍）します。
2. **Visual Studio でフォルダーを開く**
   - Visual Studio を起動し、「ローカル フォルダーを開く」から、クローンしたディレクトリ（大元の CMakeLists.txt が存在するルートフォルダ）を選択します。  
   ※旧来のソリューションファイル（.sln）は使用しません。
3. **CMake の自動構成を待つ**
   - フォルダーを開くと、Visual Studio がバックグラウンドで自動的に CMake の構成（必要なライブラリの取得やパスの解決）を開始します。出力ウィンドウに「CMake 生成が完了しました。」と表示されるまでお待ちください。
4. **ビルド構成の選択**
   - 本デモの最大パフォーマンス（10,000機以上のシミュレーション）を確認するため、上部のビルド構成プルダウンから **`x64-Release`** を選択することを推奨します。  
   （`x64-Debug` ビルドでは最適化が効かず、FPSが低下する場合があります）  
   ※プルダウンに `x64-Release` が存在しない場合は、「構成の管理」から緑色の追加ボタン（＋）を押して `x64-Release` を追加・保存してください。
5. **実行（F5）**
   - 画面上部の「スタートアップ アイテム」（再生ボタンの横）のプルダウンメニューから `DroneShow_App.exe` を選択し、クリック（または F5 キー押下）するだけで、ビルドと実行が開始されます。
6. **ユニットテストの実行**
   - 各種ユニットテスト（Google Test）を実行・確認したい場合は、スタートアップ アイテムを `DroneShow_Test.exe` に切り替えて実行（F5）してください。
